...
serve(async (req) => {
  ...
  try {
    ...
    const productsContext = products?.map(p => 
      `${p.name} (${p.item_code}): ${p.description || 'Sem descrição'}. Categoria: ${p.categories?.name || 'N/A'}. Preço: ${p.price ? `R$ ${p.price}` : 'Sob consulta'}.`
...
- Mantenha respostas concisas e diretas`;
    ...
    console.log('Chamando Lovable AI Gateway para chat...');
